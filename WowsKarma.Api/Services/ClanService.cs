...
public sealed class ClanService
{
	...
	public async Task<Clan?> GetClanAsync(uint clanId, bool includeMembers = false, CancellationToken ct = default)
	{
		...
		if (updateMembers && clan is not null)
		{
...
#pragma warning restore CA1841
		...
		{
			Player dbPlayer = (await _vortex.FetchAccountAsync(id, ct) ?? throw new InvalidOperationException($"Player {id} not found.")).ToDbModel();
