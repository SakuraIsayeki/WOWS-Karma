...
export default function PlayerSearchResults({display, username}: { display: boolean, username: string }) {
    const playerClient = container.resolve(PlayerClient);
    const { cache, mutate, ...extraConfig } = useSWRConfig()

    const path = `/api/player/search/${username}`;
    const { data, error } = useSWR(display ? path : null, username => playerClient.searchPlayers(username));
